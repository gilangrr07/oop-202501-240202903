...
public class CashierView extends BorderPane {
    ...
    private TextField paymentField, quantityField, searchField;
    private Label totalLabel, itemCountLabel;
    private final ObservableList<Product> productList;
    private final ObservableList<Product> filteredProductList;
    private final ObservableList<CartItem> cartList;
    ...
    public CashierView() {
        ...
        this.filteredProductList = FXCollections.observableArrayList();
    ...
    private VBox createProductBox() {
        ...
        TableColumn<Product, ?>[] productCols = new TableColumn[]{kodeCol, namaCol, hargaCol, stokCol};
        productTable.getColumns().addAll(productCols);
        ...
        box.getChildren().addAll(headerBox, searchBox, productTable, addBox);
    ...
    private VBox createSearchBox() {
        ...
        searchLayout.setAlignment(javafx.geometry.Pos.CENTER_LEFT);

        Label searchLabel = new Label("🔍 Cari Produk:");
        searchLabel.setStyle("-fx-font-weight: bold;");

        searchField = new TextField();
        searchField.setPromptText("Ketik kode, nama, atau kategori produk...");
        searchField.setPrefHeight(35);
        HBox.setHgrow(searchField, Priority.ALWAYS);

        // Real-time search dengan TextInputListener
        searchField.textProperty().addListener((obs, oldVal, newVal) -> {
            applySearch();
        });

        Button clearSearchButton = new Button("✕ Hapus");
        clearSearchButton.setStyle("-fx-background-color: #757575; -fx-text-fill: white;");
        clearSearchButton.setOnAction(e -> {
            searchField.clear();
            applySearch();
        });

        searchLayout.getChildren().addAll(searchLabel, searchField, clearSearchButton);
        box.getChildren().add(searchLayout);
    ...
    private void applySearch() {
        ...
        if (searchText.isEmpty()) {
            ...
            filteredProductList.setAll(
                    productList.stream()
                            .filter(product -> product.getKode().toLowerCase().contains(searchText) ||
                                    product.getNama().toLowerCase().contains(searchText) ||
                                    (product.getKategori() != null
                                            && product.getKategori().toLowerCase().contains(searchText)))
    ...
    private VBox createCartBox() {
        ...
        namaCol.setCellValueFactory(
                cellData -> new javafx.beans.property.SimpleStringProperty(cellData.getValue().getProduct().getNama()));
        ...
        priceCol.setCellValueFactory(
                cellData -> new javafx.beans.property.SimpleDoubleProperty(cellData.getValue().getProduct().getHarga())
                        .asObject());
        ...
        subtotalCol.setCellValueFactory(
                cellData -> new javafx.beans.property.SimpleDoubleProperty(cellData.getValue().getSubtotal())
                        .asObject());
        ...
        @SuppressWarnings("unchecked")
        TableColumn<CartItem, ?>[] cartCols = new TableColumn[]{namaCol, qtyCol, priceCol, subtotalCol, actionCol};
